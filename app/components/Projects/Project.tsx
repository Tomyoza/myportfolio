...
const Project = ({ thumbnail, title, link, description, languageIcons }: ProjectProps) => {
  return (
    ...
    p-5 md:p-[18px] gap-[30px]'>
      <Image src={thumbnail} alt={'Thumnail for ${title}'} width={392} height={230} className='w-full
      ...
      <div className='flex flex-col gap-[11px]'>
        <div className='flex justify-between items-center'>
          ...
          bg-opacity-[24p%] rounded text-[14px]'>
            ...
            <img src='link_dark_arrow.svg' alt="link dark arrow" className='hidden dark:block'/>
          </a>
        </div>
        <p className='line-clamp-2 text-sm md:text-base'>{description}</p>
        <div className='flex flex-row gap-[11px]'>
          {languageIcons.map((icon, iconID) => (<img src={icon} alt='Language icon' key={iconID}/> ))}
        </div>
      </div>
    </div>
  )
